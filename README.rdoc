...
3/31/14
	...
	Problems to fix:
		...
		(13) show groupings on the users index page
		(15) search for body on posts and comments
		(16) whole nav bar shifts when screen is resized
		(17) default alerts & notices turned off for devise
		(18) database of images only "image show"
		(19) make sure it looks good on multiple browsers
		(20) fix the search bar -- can't search body for posts & comments, can't search groups if there are no users
