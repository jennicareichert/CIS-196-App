...
3/31/14
	...
	Problems to fix:
		(1) Make it so users can't sign up without first selecting a group to join
		(2) Add Filepicker gem install 
		(3) Add Twilio (SMS texting notificaitons) gem install
		(5) Change the views on index for all items so that it looks more appealing and well-spaced.
		(6) Want to be able to view a post with all of its attached comments below it -- not 2 separate pages
		(7) Only see posts/comments FOR the given group dashboard (not a user dashboard, but a grouping dashboard)
		(8) Invite people (current or new users) to join a group
		(9) Group privacy -- can't join without a validation from an existing member
		(10) Make the "Leave a comment" link on the posts page be specifically a linked comment to that particular post
		(11) Allow user groups to set their own group background (similar to how I have now)
		(12) Add in to groups.controller.rb the "show me more" using AJAX to see more than 15 posts
		(13) Add search bar to the navigation & shift "Dashboard" to float left
