	...
	Using Ruby Gems:
		Can search for Ruby Gems on command line:
			...
			Can manage gems with the gem file in sublime.
				We want gem "devise', '3.8'
				bundle install or bundle update
				bundle open outdated

				rails g devise:install

		rails generate migration RemoveEmailFromUsers emailstring
		rails destroy migration Devise
		rake db:migrate
		
			*If you get an error page, try reloading the rails server*

		Creating information for user validation.

			<c class="notice"><%= notice %></c>
			<c class = alerts"><%= alert %></c>

			make it so that your login shows only when logged out, etc.
			<%= if user_signed_in?

			add to the application controller
			before_filter :authenticate_user:

		add to the posts_controller.rb
			if @post.user_id current_user.id
				render  'new'
			else 

		give the user more power to view information
			rails generate devise:views
			use registrations/new.html.erb
				edit this one to ask for user name
